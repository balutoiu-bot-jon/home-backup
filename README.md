...
The parent CronJob Job template **must** set `terminationGracePeriodSeconds: 180` or greater. Home-backup fails before creating snapshots when it is absent or smaller. The minimum covers the 120-second cleanup budget, the child's 30-second shutdown grace, and a 30-second margin:
